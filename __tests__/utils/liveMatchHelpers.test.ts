...
describe("getActionDescription", () => {
  ...
  describe("tirs réussis", () => {
    it("retourne (+3) pour un tir à 3 points réussi", () => {
      const action = makeAction({ points: 3, specification: ShotSpecification.MADE });
      expect(getActionDescription(action, "Dupont")).toBe("10 - Dupont (+3)");
    });

    it("retourne (+2) pour un tir à 2 points réussi", () => {
      const action = makeAction({ points: 2, specification: ShotSpecification.MADE });
      expect(getActionDescription(action, "Dupont")).toBe("10 - Dupont (+2)");
    });

    it("retourne (+1) pour un lancer franc réussi", () => {
      const action = makeAction({ points: 1, specification: ShotSpecification.MADE });
      expect(getActionDescription(action, "Dupont")).toBe("10 - Dupont (+1)");
    });
  });

  describe("tirs ratés", () => {
    it("retourne Raté (3pts) pour un 3 points raté", () => {
      const action = makeAction({ points: 3, specification: ShotSpecification.MISSED });
      expect(getActionDescription(action, "Dupont")).toBe("10 - Dupont Raté (3pts)");
    });

    it("retourne Raté (2pts) pour un 2 points raté", () => {
      const action = makeAction({ points: 2, specification: ShotSpecification.MISSED });
      expect(getActionDescription(action, "Dupont")).toBe("10 - Dupont Raté (2pts)");
    });

    it("retourne Raté (LF) pour un lancer franc raté", () => {
      const action = makeAction({ points: 1, specification: ShotSpecification.MISSED });
      expect(getActionDescription(action, "Dupont")).toBe("10 - Dupont Raté (LF)");
    });
  });

  describe("adversaire générique (numéro 9999)", () => {
    it("affiche +3 sans parenthèses pour l'adversaire générique", () => {
      ...
      expect(getActionDescription(action, "Adversaire")).toBe("Adversaire +3");
    });

    it("affiche +2 sans parenthèses pour l'adversaire générique", () => {
      const action = makeAction({
        player_number: 9999,
        team: Team.OPPONENT,
        points: 2,
      ...
      expect(getActionDescription(action, "Adversaire")).toBe("Adversaire +2");
    });
  });

  describe("rebonds", () => {
    it("retourne Rebond Déf pour un rebond défensif", () => {
      const action = makeAction({
        action_type: ActionType.REBOUND,
        specification: ReboundSpecification.DEFENSIVE,
      });
      expect(getActionDescription(action, "Martin")).toBe("10 - Martin Rebond Déf");
    });

    it("retourne Rebond Off pour un rebond offensif", () => {
      const action = makeAction({
        action_type: ActionType.REBOUND,
        specification: ReboundSpecification.OFFENSIVE,
      });
      expect(getActionDescription(action, "Martin")).toBe("10 - Martin Rebond Off");
    });
  });
  ...
  describe("autres actions", () => {
    it("retourne Faute avec le bon format", () => {
      ...
      expect(getActionDescription(action, "Dupont")).toBe("10 - Faute Dupont");
    });
  ...
  describe("préfixe numéro", () => {
    it("n'affiche pas de préfixe si player_number est undefined", () => {
    ...
    it("n'affiche pas de préfixe si player_number est null", () => {
