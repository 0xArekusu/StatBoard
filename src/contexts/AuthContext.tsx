...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      logError('AuthProvider', '❌ Failed to configure Google Sign-In', { error });
    }
    ...
    supabase.auth.getSession().then(({ data: { session }, error }) => {
      if (error) {
        logError('AuthProvider', '❌ Failed to fetch initial session', { error: error.message });
      } else {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      if (userInfo.data?.idToken) {
        logInfo('AuthProvider', '🎫 ID token obtained, authenticating with Supabase', {
      ...
      logError('AuthProvider', '❌ No ID token present in Google Sign-In response');
  ...
  const signOut = async () => {
    ...
    try {
      ...
      logWarn('AuthProvider', '⚠️ Google sign out failed (may not be signed in)', { error });
    }
    ...
    try {
      ...
      if (error) {
        logError('AuthProvider', '❌ Supabase sign out failed', { error: error.message });
      } else {
