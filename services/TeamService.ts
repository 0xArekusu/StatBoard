...
export class TeamService {
  ...
  async createTeam(data: CreateTeamData, userId: string) {
    ...
    if (!data.name || data.name.trim().length === 0) {
      return { success: false, error: i18n.t("teamService.nameRequired") };
    }

    // Check if user is a member of the club
    const isMember = await this.clubMemberRepository.findByClubAndUser(data.clubId, userId);
    if (!isMember) {
      return { success: false, error: i18n.t("teamService.notClubMember") };
    }
    ...
    if (this.subscriptionService) {
      ...
      if (!limitCheck.allowed) {
        return {
          success: false,
          error: limitCheck.error || i18n.t("teamService.teamLimitReached"),
        };
      }
    }
    ...
    if (teamCount >= MAX_TEAMS_PER_USER_PER_CLUB) {
      return {
        success: false,
        error: i18n.t("teamService.perUserTeamLimitReached", { max: MAX_TEAMS_PER_USER_PER_CLUB }),
      };
    }

    const team = await this.teamRepository.create(data, userId);
    if (!team) {
      return { success: false, error: i18n.t("teamService.createFailed") };
    }
  ...
  async updateTeam(id: string, data: UpdateTeamData, userId: string) {
    ...
    if (!team) {
      return { success: false, error: i18n.t("teamService.notFound") };
    }
    ...
    if (!updatedTeam) {
      return { success: false, error: i18n.t("teamService.updateFailed") };
    }
  ...
  async updateTeamStatus(teamId: string, status: TeamStatus, clubOwnerId: string) {
    ...
    if (!updatedTeam) {
      return { success: false, error: i18n.t("teamService.statusUpdateFailed") };
    }
  ...
  async toggleTeamActive(teamId: string, clubOwnerId: string) {
    ...
    if (!updatedTeam) {
      return { success: false, error: i18n.t("teamService.toggleActiveFailed") };
    }
  ...
  async deleteTeam(id: string, userId: string) {
    ...
    if (team.ownerId !== userId) {
      return { success: false, error: i18n.t("teamService.noDeletePermission") };
    }

    const deleted = await this.teamRepository.delete(id);
    if (!deleted) {
      return { success: false, error: i18n.t("teamService.deleteFailed") };
    }
  ...
  async enforceTeamLimits(
    ...
    try {
      ...
      return {
        success: false,
        error: i18n.t("teamService.enforceLimitsFailed"),
      };
    }
  }
