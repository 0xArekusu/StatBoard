...
export default function MatchDetailsScreen() {
  ...
  const [selectedTeam, setSelectedTeam] = useState<"A" | "B" | "BOTH">(
    teamMode
  );
  ...
  const [courtFilterActionType, setCourtFilterActionType] = useState<string[]>(
    []
  ); // ["shot", "rebound", "foul"]
  ...
  useEffect(() => {
    logInfo(
      "MatchDetailsScreen",
      "📱 Screen mounted - User arrived on match details",
      {
        teamA,
        teamB,
        scoreA,
  ...
  const calculatePlayerStats = (playerNum: number, playerTeam: "A" | "B") => {
    ...
    const shots = playerActions.filter(
      (action) => action.type === ActionType.SHOT
    );
    ...
    const onePtMade = onePtShots.filter(
      (a) => a.specification === ShotSpecification.MADE
    ).length;
    const twoPtMade = twoPtShots.filter(
      (a) => a.specification === ShotSpecification.MADE
    ).length;
    const threePtMade = threePtShots.filter(
      (a) => a.specification === ShotSpecification.MADE
    ).length;
    ...
    const rebounds = playerActions.filter(
      (action) => action.type === ActionType.REBOUND
    );
    ...
    const fouls = playerActions.filter(
      (action) => action.type === ActionType.FOUL
    );
    ...
    const shotPercentage =
      shots.length > 0
        ? Math.round((madeShots.length / shots.length) * 100)
        : 0;
  ...
  const filteredCourtActions = useMemo(() => {
    return actions.filter((action) => {
      // Team filter
      if (
        courtFilterTeams.length > 0 &&
        !courtFilterTeams.includes(action.team)
      ) {
        return false;
      }
      ...
      // Action type filter
      if (
        courtFilterActionType.length > 0 &&
        !courtFilterActionType.includes(action.type)
      ) {
        return false;
      }
  ...
  return (
    ...
    >
      ...
      <ScrollView style={styles.mainScroll}>
        ...
        <View
          style={[styles.scoreSummary, { backgroundColor: colors.surface }]}
        >
          <View style={styles.teamScoreContainer}>
            <Text
              style={[styles.teamNameSmall, { color: colors.text.secondary }]}
            >
              {teamA}
            </Text>
          ...
          <Text
            style={[styles.scoreSeparator, { color: colors.text.disabled }]}
          >
            -
          </Text>
          <View style={styles.teamScoreContainer}>
        ...
        >
          <TouchableOpacity
            style={[
              ...
              activeTab === "court" && [
                styles.tabActive,
                { borderBottomColor: STATUS_COLORS.success },
              ],
            ]}
        ...
        {activeTab === "court" && (
          <View
            style={[styles.courtSection, { backgroundColor: colors.surface }]}
          >
            ...
            <View style={styles.courtAndStatsContainer}>
              ...
              >
                <Text
                  style={[
                    styles.liveStatsTitle,
                    { color: colors.text.primary },
                  ]}
                >
                  {teamMode === "BOTH" ? teamA : "Stats"}
                </Text>

                {/* Shooting Stats */}
                {(courtFilterActionType.length === 0 ||
                  courtFilterActionType.includes(ActionType.SHOT)) && (
                  <>
                    <View
                      style={[
                        styles.liveStatGroup,
                        { borderBottomColor: colors.border },
                      ]}
                    >
                      <View
                      ...
                      >
                        ...
                        >
                          <Path
                            d="M 12 0 A 12 12 0 0 1 12 24 Z"
                            fill="#4CAF50"
                          />
                          <Path
                            d="M 12 0 A 12 12 0 0 0 12 24 Z"
                            fill="#F44336"
                          />
                        </Svg>
                        <Text
                          style={[
                            styles.liveStatGroupLabel,
                            { color: colors.text.secondary },
                          ]}
                        >
                          Tirs
                      ...
                      >
                        {(() => {
                          const actionsToUse =
                            teamMode === "BOTH"
                              ? filteredCourtActions.filter(
                                  (a) => a.team === "A"
                                )
                              : filteredCourtActions;
                          const shots = actionsToUse.filter(
                            (a) => a.type === ActionType.SHOT
                          );
                          ...
                          const percentage =
                            shots.length > 0
                              ? Math.round((made / shots.length) * 100)
                              : 0;
                          ...
                          {(() => {
                            const actionsToUse =
                              teamMode === "BOTH"
                                ...
                                : filteredCourtActions;
                            const onePtShots = actionsToUse.filter(
                              (a) =>
                                a.type === ActionType.SHOT && a.points === 1
                            );
                            const onePtMade = onePtShots.filter(
                              (a) => a.specification === ShotSpecification.MADE
                            ).length;
                            const onePtPercentage =
                              onePtShots.length > 0
                                ? Math.round(
                                    (onePtMade / onePtShots.length) * 100
                                  )
                                : 0;
                          ...
                          {(() => {
                            const actionsToUse =
                              teamMode === "BOTH"
                                ...
                                : filteredCourtActions;
                            const twoPtShots = actionsToUse.filter(
                              (a) =>
                                a.type === ActionType.SHOT && a.points === 2
                            );
                            const twoPtMade = twoPtShots.filter(
                              (a) => a.specification === ShotSpecification.MADE
                            ).length;
                            const twoPtPercentage =
                              twoPtShots.length > 0
                                ? Math.round(
                                    (twoPtMade / twoPtShots.length) * 100
                                  )
                                : 0;
                          ...
                          {(() => {
                            const actionsToUse =
                              teamMode === "BOTH"
                                ...
                                : filteredCourtActions;
                            const threePtShots = actionsToUse.filter(
                              (a) =>
                                a.type === ActionType.SHOT && a.points === 3
                            );
                            const threePtMade = threePtShots.filter(
                              (a) => a.specification === ShotSpecification.MADE
                            ).length;
                            const threePtPercentage =
                              threePtShots.length > 0
                                ? Math.round(
                                    (threePtMade / threePtShots.length) * 100
                                  )
                                : 0;
                ...
                {/* Rebounds Stats */}
                {(courtFilterActionType.length === 0 ||
                  courtFilterActionType.includes(ActionType.REBOUND)) && (
                  <View
                    style={[
                      styles.liveStatGroup,
                      { borderBottomColor: colors.border },
                  ...
                  >
                    ...
                    >
                      {(() => {
                        const actionsToUse =
                          teamMode === "BOTH"
                            ? filteredCourtActions.filter((a) => a.team === "A")
                            : filteredCourtActions;
                        return actionsToUse.filter(
                          (a) => a.type === ActionType.REBOUND
                        ).length;
                      })()}
                    </Text>
                    <View style={styles.liveStatDetail}>
                ...
                {/* Fouls Stats */}
                {(courtFilterActionType.length === 0 ||
                  courtFilterActionType.includes(ActionType.FOUL)) && (
                  <View
                    style={[
                      styles.liveStatGroup,
                      { borderBottomColor: colors.border },
                      ...
                      >
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            return actionsToUse.filter(
                              (a) =>
                                a.type === ActionType.FOUL &&
                                a.specification === FoulSpecification.PERSONAL
                            ).length;
                          })()}
                        </Text>
                      </View>
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            return actionsToUse.filter(
                              (a) =>
                                a.type === ActionType.FOUL &&
                                a.specification === FoulSpecification.TECHNICAL
                            ).length;
                          })()}
                        </Text>
                      </View>
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            return actionsToUse.filter(
                              (a) =>
                                a.type === ActionType.FOUL &&
                                a.specification === FoulSpecification.PENALITY
                            ).length;
                          })()}
                        </Text>
                      </View>
                ...
                {ACTION_DEFINITIONS.filter(
                  (action) =>
                    ![
                      ...
                      ActionType.REBOUND,
                      ActionType.FOUL,
                    ].includes(action.id as ActionType)
                )
                  .filter(
                    (action) =>
                      courtFilterActionType.length === 0 ||
                      courtFilterActionType.includes(action.id)
                  )
              ...
              {teamMode === "BOTH" ? (
                  ...
                  {(courtFilterActionType.length === 0 ||
                    ...
                    <>
                      ...
                      >
                        ...
                        >
                          {(() => {
                            const actionsTeamB = filteredCourtActions.filter(
                              (a) => a.team === "B"
                            );
                            const shots = actionsTeamB.filter(
                              (a) => a.type === ActionType.SHOT
                            );
                            const made = shots.filter(
                              (a) => a.specification === ShotSpecification.MADE
                        ...
                        <View style={styles.liveStatDetail}>
                          ...
                          >
                            ...
                            {(() => {
                              ...
                              const onePtShots = actionsTeamB.filter(
                                (a) =>
                                  a.type === ActionType.SHOT && a.points === 1
                              );
                              const onePtMade = onePtShots.filter(
                          ...
                          >
                            ...
                            {(() => {
                              ...
                              const twoPtShots = actionsTeamB.filter(
                                (a) =>
                                  a.type === ActionType.SHOT && a.points === 2
                              );
                              const twoPtMade = twoPtShots.filter(
                          ...
                          >
                            ...
                            {(() => {
                              ...
                              const threePtShots = actionsTeamB.filter(
                                (a) =>
                                  a.type === ActionType.SHOT && a.points === 3
                              );
                              const threePtMade = threePtShots.filter(
                ...
                <View style={styles.rightPanel}>{/* Empty for now */}</View>
              )}
            </View>
          </View>
        )}
                ...
                {SORT_OPTIONS.map((option) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      { backgroundColor: colors.surfaceVariant },
                      sortBy === option.key && {
                        backgroundColor: SPORT_COLORS.basketball.orange,
                      },
                    ]}
                ...
                >
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.playerPoints,
                        { color: SPORT_COLORS.basketball.orange },
                      ]}
                    >
                    ...
                    {player.stats.shots > 0 && (
                      <View style={styles.statSection}>
                        ...
                        <View style={styles.statRow}>
                          ...
                          >
                            {player.stats.madeShots}/{player.stats.shots} (
                            {player.stats.shotPercentage}%)
                          </Text>
                        </View>
                        <View style={styles.statRow}>
                          <Text
                          ...
                          >
                            {player.stats.onePtMade}/{player.stats.onePtTotal} (
                            {player.stats.onePtPercentage}%)
                          </Text>
                        </View>
                        <View style={styles.statRow}>
                          <Text
                          ...
                          >
                            {player.stats.twoPtMade}/{player.stats.twoPtTotal} (
                            {player.stats.twoPtPercentage}%)
                          </Text>
                        </View>
                        <View style={styles.statRow}>
                          <Text
                          ...
                          >
                            {player.stats.threePtMade}/
                            {player.stats.threePtTotal} (
                            {player.stats.threePtPercentage}%)
                          </Text>
                        </View>
                      </View>
                    )}
                      ...
                      const otherStatsConfig = [
                        {
                          key: "rebounds",
                          label: "Rebonds",
                          detail: `(Off: ${player.stats.offensiveRebounds} / Def: ${player.stats.defensiveRebounds})`,
                        },
                        {
                          key: "assists",
                          label:
                            ACTION_DEFINITIONS.find(
                              (a) => a.id === ActionType.ASSIST
                            )?.label || "Passes",
                          detail: null,
                        },
                        {
                        ...
                        },
                        {
                          ...
                          detail:
                            player.stats.fouls > 0
                              ? `(Pers: ${player.stats.personalFouls} / Tech: ${player.stats.technicalFouls} / Anti: ${player.stats.antisportiveFouls} / Disq: ${player.stats.disqualifyingFouls})`
                              : null,
                        },
                      ];
